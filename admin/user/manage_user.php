			...
			<form action="" id="manage-user">	
				...
				<div class="form-group col-6">
					...
					<select name="type" id="type" class="custom-select">
						<option value="1" <?php echo isset($meta['type']) && $meta['type'] == 1 ? 'selected' : '' ?>>Người quản trị website</option>
						<option value="2" <?php echo isset($meta['type']) && $meta['type'] == 2 ? 'selected' : '' ?>>Nhân viên quản lý</option>
					</select>
				</div>
				<div class="form-group col-6">
	...
	<div class="card-footer">
			<div class="col-md-12">
				<div class="row">
					...
					<a class="btn btn-sm btn-secondary" href="./?page=user/list">Hủy bỏ</a>
				</div>
			</div>
		</div>
</div>
		...
		$.ajax({
			...
			success:function(resp){
				if(resp ==1){
					...
					el.addClass('alert-danger').text('Tài khoản đã tồn tại')
            		_this.prepend(el)
            		el.show('slow')
            		$('html,body').scrollTop(0)
            return false;
