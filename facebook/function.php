...
function loginFromSocialCallBack($socialUser) {
    ...
    $result = mysqli_query($con, "Select `id`,`username`,`email`,`fullname` from `client_fb_list` WHERE `email` ='" . $socialUser['email'] . "'");
    if ($result->num_rows == 0) {
        $result = mysqli_query($con, "INSERT INTO `client_fb_list` (`fullname`,`email`, `status`, `created_time`, `last_updated`) VALUES ('" . $socialUser['name'] . "', '" . $socialUser['email'] . "', 1, " . time() . ", '" . time() . "');");
        if (!$result) {
