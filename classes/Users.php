...
Class Users extends DBConnection {
			...
			if($qry){
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						$resp['msg']=" Hình ảnh không tải lên được do loại tệp không hợp lệ.";
					}else{
						$new_height = 200; 
						$new_width = 200; 
						...
						$resp['msg']=" Hình ảnh không tải lên được vì lý do không xác định.";
						}
					}
	...
	public function save_sales_agent(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = " Tên Tài khoản đã được sử dụng";
		}else{
			if(empty($id)){
			...
			if($save){
				...
				if(empty($id)){
					if(strpos($_SERVER['HTTP_REFERER'], 'admin/sales_agent/create_account_salesman.php') > -1){
						$resp['msg'] = " Tài khoản của Nhân viên bán hàng đã được đăng ký thành công.";
					}else{
						$resp['msg'] = " Tài khoản của Nhân viên bán hàng đã được đăng ký thành công.";
					}
				}else{
					if($this->settings->userdata('login_type') == 2){
						$resp['msg'] = " Chi tiết tài khoản của Nhân viên bán hàng đã được cập nhật thành công.";
					}else{
						$resp['msg'] = " Chi tiết Tài khoản của Nhân viên bán hàng đã được cập nhật thành công.";
					}	
				}
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					$allowed = array('image/png','image/jpeg');
					if(!in_array($type,$allowed)){
						$resp['msg'].=" Hình ảnh không thể tải lên do loại tệp không hợp lệ.";
					}else{
						$new_height = 200; 
						$new_width = 200; 
						...
						$resp['msg'].=" Không thể tải lên hình ảnh do một lý do không xác định.";
						}
					}
	...
	public function delete_sales_agent(){
		extract($_POST);
		$qry = $this->conn->query("UPDATE vendor_list set delete_flag = 1 where id = $id");
		if($qry){
			$this->settings->set_flashdata('success',' Tài khoản nhân viên bán hàng đã được xóa thành công.');
			$resp['status'] = 'success';
		}else{
			$resp['status'] = 'failed';
...
switch ($action) {
	...
	break;
	case 'save_sales_agent':
		echo $users->save_sales_agent();
	break;
	case 'delete_sales_agent':
		echo $users->delete_sales_agent();
	break;
	case 'save_client':
