...
class Login extends DBConnection {
	...
	public function login(){
		...
		if($result->num_rows > 0){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = 'Tên đăng nhập hoặc mật khẩu không đúng, vui lòng thử lại.';
	...
	public function login_sales_agent(){
		extract($_POST);
		$password = md5($password);
	...
	public function logout_sales_agent(){
		if($this->settings->sess_des()){
			redirect('sales_agent');
		}
	}
...
switch ($action) {
	...
	case 'logout_client':
		...
		break;
	case 'login_sales_agent':
		echo $auth->login_sales_agent();
		break;
	case 'logout_sales_agent':
		echo $auth->logout_sales_agent();
		break;
	default:
		echo $auth->index();
		break;
