...
Class Master extends DBConnection {
	...
	function save_category(){
		...
		if($check > 0){
			...
			if($save){
				...
				$resp['msg'] = " Đã cập nhật danh mục thành công.";
			}else{
				$resp['status'] = 'failed';
				$resp['err'] = $this->conn->error."[{$sql}]";
			}
			...
			if($save){
				...
				if(isset($_FILES['img']) && $_FILES['img']['tmp_name'] != ''){
					...
					if(!in_array($type,$allowed)){
						$resp['msg']=" Không thể tải lên hình ảnh do loại tệp không hợp lệ.";
					}else{
						...
						$resp['msg']=" Hình ảnh không thể tải lên do một lý do không xác định.";
						}
					}
				...
				$resp['status'] = 'failed';
				if(empty($id))
					$resp['msg'] = " Không lưu được sản phẩm.";
				else
	...
	function add_to_cart(){
		...
		if($save){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = " Không thể thêm sản phẩm vào giỏ hàng.";
	...
	function delete_cart(){
		...
		if($del){
			$resp['status'] = 'success';
			$resp['msg'] = " Danh sách Giỏ hàng đã được xóa thành công.";
		}else{
			$resp['status'] = 'failed';
			$resp['msg'] = " Không thể xóa sản phẩm trong Giỏ hàng.";
			$resp['error'] = $this->conn->error;
		}
		if($resp['status'] =='success'){
			$this->settings->set_flashdata('success',$resp['msg']);
	...
	function place_order(){
		...
		if(!$has_failed){
			...
			$resp['status'] = 'failed';
			$resp['msg'] = " Không đặt được đơn hàng";
			$resp['error'] = $this->conn->error;
			if(count($inserted) > 0){
